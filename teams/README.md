...
> ⚖️ **目的/技术分层**：目的问题（为什么做、设计/世界观/体验）与技术问题（怎么做/实现）都由 **Manager 经 wayfinder 组织**——Manager 是 wayfinder 的主人，grilling 归 Manager（决定问什么），不归秘书。秘书 = 人机对话**执行接口**（转述问题、回传答案、编译 GDD 留档），不拥有 grill 的决定权。
...
## Worker 汇报纪律（团队搭建时注入，非可选尾巴）

> **汇报是任务的一部分，不是可选尾巴。** 每个被拉起的 Worker agent，拉起提示词里**默认注入汇报纪律**（作为角色提示词的一部分，写进 initialPrompt / 派单文案）。团队搭建阶段就把这条教给 worker，避免"干完不 send、来回问进度"的低效。

### 默认注入的汇报纪律（角色提示词片段，直接放 initialPrompt）

```
完成任务后：①写回执文件 → ②paseo send 汇报（send 内容含回执路径 + status）
遇阻塞/进展异常：随时 paseo send 同步，不等完成
```

顺序铁律：**先写回执文件 → 再 paseo send**；Manager 验收以**回执文件**为准，不轻信口头"完成"。降级：send 失败时把汇报写进回执文件并明确告知 Manager 需轮询。

### 角色-汇报义务

| 角色 | 汇报义务 |
|---|---|
| 研究员 | 完成工单 → 回执 + paseo send（含回执路径 + status） |
| Manager | 汇总各研究员结果 → 汇报秘书/用户 |
| 秘书 | 对话自然同步（听需求、拆单、汇总回传都在对话里） |

> 与 `protocol-cmux.md` 的"验收清单三步 / 完成即回执即汇报 / send 消息格式 / 降级"保持一致——团队向导负责**拉起时注入**，派单协议负责**任务单执行时**落实。
